...
void OLED_ShowFloat(uint8_t Line, uint8_t Column, float Number, uint8_t Length)
{
	uint8_t i = 0;
	for (i = 0; i < Length; i++) {
		OLED_ShowChar(Line, Column + i, (uint32_t)Number/ OLED_Pow(10, Length - i - 1) % 10 + '0');
	}
	OLED_ShowChar(Line, Column + i + 1, '.');
	i++;
	OLED_ShowChar(Line, Column + i + 1, (uint32_t)(Number * 10) / OLED_Pow(10, Length - i + 1) % 10 + '0');
	i++;
	OLED_ShowChar(Line, Column + i + 1, (uint32_t)(Number * 100) / OLED_Pow(10, Length - i + 2) % 10 + '0');
}
...
/**
  ...
  * @param  Line ��ʼ��λ�ã���Χ��1~4
  * @param  Column ��ʼ��λ�ã���Χ��1~16
  * @param  Number Ҫ��ʾ�����֣���Χ��-2147483648~214748364
  * @param  Length Ҫ��ʾ���ֵĳ��ȣ���Χ��1~10
  * @retval ��
  */
...
void OLED_ShowSignedFloat(uint8_t Line, uint8_t Column, float Number, uint8_t Length)
{
	uint8_t i;
	float Number1;
	if (Number >= 0)
	...
	for (i = 0; i < Length; i++)							
	{
		OLED_ShowChar(Line, Column + i + 1, (int32_t)Number1 / OLED_Pow(10, Length - i - 1) % 10 + '0');
	}
	OLED_ShowChar(Line, Column + i + 1, '.');
	i++;
	OLED_ShowChar(Line, Column + i + 1, (int32_t)(Number * 10) / OLED_Pow(10, Length - i + 1) % 10 + '0');
	i++;
	OLED_ShowChar(Line, Column + i + 1, (int32_t)(Number * 100) / OLED_Pow(10, Length - i + 2) % 10 + '0');
}
